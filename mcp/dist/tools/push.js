...
import os from "os";
import { detectPlatformFromUrl, parseGitUrl, getToken, getConfigRepoUrl, getGlobalConfigPath, getLocalConfigPath, configExists, ensureDir, } from "../platform.js";
const CONFIG_REPO = "claude-code-config-sync";
const TEMP_DIR = path.join(os.tmpdir(), CONFIG_REPO);
export async function gitPush(scope) {
    let token = "";
    try {
        // 1. 获取当前仓库的远程 URL
        const git = simpleGit();
